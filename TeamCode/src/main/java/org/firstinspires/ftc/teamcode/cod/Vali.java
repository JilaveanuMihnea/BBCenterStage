...
public class Vali extends LinearOpMode {
    ...
    private void movement( float forward, float strafe, float rotation){
        ...
        double[] power;
            power = new double[4];
            rotation *= -1;
            power[0] = (-forward -strafe + rotation) * 1f;   //+
            power[1] = (-forward - strafe - rotation) * 1f;   //-
            power[2] = (-forward + strafe + rotation) * 1f;   //-
            power[3] = (+forward - strafe + rotation) * 1f;
        //+
        // applying the power
        for (int i = 0; i < 4; i++) {
    ...
    private void militaru(float button, float button1){
        mSlider.setPower(button - button1);
    }

    private void beam(boolean button1, boolean button2){
        if(button1) {
            sBeam.setPosition(0.7f);
        }
        else if (button2) {
